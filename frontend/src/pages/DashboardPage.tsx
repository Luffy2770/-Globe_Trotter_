...
import { Search, Calendar, MapPin, DollarSign, Clock, Compass, PlusCircle, ArrowUpRight, Sparkles } from 'lucide-react';
...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 space-y-8 animate-scale-up">
      {/* Hero Banner with Smooth Image Animation */}
      {dashboardData?.banner && (
        <div className="relative rounded-3xl overflow-hidden bg-slate-900 text-white shadow-xl group">
          <img
            ...
            alt="Hero Banner"
            className="w-full h-60 object-cover object-center opacity-40 group-hover:scale-105 transition duration-700 ease-out"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-slate-950/90 via-slate-900/60 to-transparent p-8 flex flex-col justify-center max-w-xl space-y-3">
            <span className="px-3 py-1 bg-white/20 backdrop-blur-md text-white text-[11px] font-bold rounded-full w-fit flex items-center space-x-1">
            ...
            <button
              ...
              className="w-fit mt-2 py-3 px-5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-2xl flex items-center space-x-2 shadow-lg shadow-blue-600/25 transition active:scale-[0.98]"
            >
      ...
      {dashboardData?.top_regional_selections?.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-medium">Curated travel recommendations</span>
          </div>
              ...
              >
                <div className="overflow-hidden rounded-xl mb-2.5">
                  <img
                    src={city.image_url}
                    alt={city.name}
                    className="w-full h-24 object-cover group-hover:scale-110 transition duration-500 ease-out"
                  />
                </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="Search trips by name or destination city..."
            value={searchTerm}
